...
class Trainer(object):
    ...
    def get_custom_dataset(self, mode):
        ...
        if mode == "train":
            ...
            custom_dataset = CustomDataset(
                ...
                vis_test_dir=self.config.vis_test_dir,
                sample=self.config.train.data.custom_sample,
                vis_opt=False,  # Without visualization
    ...
    def one_step(self, images, region_scores, affinity_scores, confidence_masks, craft, optimizer, criterion, scaler, batch_syn=None, mode="train"):
    ...
    def iou_eval(self, dataset, train_step, buffer, model, test="val"):
        ...
        # Return metrics instead of logging here
        return {
            "Recall": np.round(metrics["recall"], 3),
            "Precision": np.round(metrics["precision"], 3),
            "F1-score": np.round(metrics["hmean"], 3),
        }
    ...
    def train(self, buffer_dict):
        ...
        if self.config.train.real_dataset == "custom":
            trn_real_dataset = self.get_custom_dataset('train')
        else:
        ...
        eval_loss_dataset = self.get_custom_dataset('eval_loss')

        if self.config.mode == "weak_supervision":
            eval_loss_dataset.update_model(supervision_model)
            eval_loss_dataset.update_device(supervision_device)

        eval_loss_loader = torch.utils.data.DataLoader(
            eval_loss_dataset,
            batch_size=self.config.train.batch_size,
            shuffle=False,
            num_workers=self.config.train.num_workers,
        ...
        test_loss_dataset = self.get_custom_dataset('test_loss')

        if self.config.mode == "weak_supervision":
            test_loss_dataset.update_model(supervision_model)
            test_loss_dataset.update_device(supervision_device)

        test_loss_loader = torch.utils.data.DataLoader(
            test_loss_dataset,
            batch_size=self.config.train.batch_size,
            shuffle=False,
            num_workers=self.config.train.num_workers,
        ...
        eval_loss_iterator = iter(eval_loss_loader)
        test_loss_iterator = iter(test_loss_loader)
        ...
        while train_step < whole_training_step:
            for (
                ...
                loss_value += self.one_step(images, region_scores, affinity_scores, confidence_masks, craft, optimizer, criterion, scaler)

                craft.eval()

                try:
                    eval_batch = next(eval_loss_iterator)
                    eval_images, eval_region_scores, eval_affinity_scores, eval_confidence_masks = eval_batch
                except StopIteration:
                    # If batches are exhausted, create new iterator
                    eval_loss_iterator = iter(eval_loss_loader)
                    eval_batch = next(eval_loss_iterator)
                    eval_images, eval_region_scores, eval_affinity_scores, eval_confidence_masks = eval_batch

                eval_loss_value += self.one_step(images, region_scores, affinity_scores, confidence_masks, craft, optimizer, criterion, scaler, mode="eval")

                try:
                    test_batch = next(test_loss_iterator)
                    test_images, test_region_scores, test_affinity_scores, test_confidence_masks = test_batch
                except StopIteration:
                    # If batches are exhausted, create new iterator
                    test_loss_iterator = iter(test_loss_loader)
                    test_batch = next(test_loss_iterator)
                ...
                test_loss_value += self.one_step(images, region_scores, affinity_scores, confidence_masks, craft, optimizer, criterion, scaler, mode="eval")

                end_time = time.time()
                batch_time += end_time - start_time

                if train_step > 0 and train_step % self.config.train.eval_interval == 0:
                    mean_loss = loss_value / self.config.train.eval_interval
                    eval_mean_loss = eval_loss_value / self.config.train.eval_interval
                    test_mean_loss = test_loss_value / self.config.train.eval_interval
                    loss_value = 0
                    eval_loss_value = 0
                    test_loss_value = 0
                    avg_batch_time = batch_time / self.config.train.eval_interval
                    batch_time = 0

                    print(
                        f"{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())} | "
                        f"Step: {train_step}/{whole_training_step} | "
                        f"LR: {training_lr:.8f} | "
                        f"Train Mean Loss: {mean_loss:.5f} | "
                        f"Eval Mean Loss: {eval_mean_loss:.5f} | "
                        f"Test Mean Loss: {test_mean_loss:.5f} | "
                        f"Avg Batch Time: {avg_batch_time:.5f}s"
                    )

                    # Consolidated wandb logging - prepare base metrics
                    wandb_metrics = {
                    ...
                    # validation and get evaluation metrics
                    print("Validation")
                    eval_metrics = self.iou_eval(
                        "custom_data",
                        train_step,
                    ...
                    metrics = {
                        ...
                        "eval_f1_score": eval_metrics["F1-score"],
                        "test_recall": test_metrics["Recall"],
                        "test_precision": test_metrics["Precision"],
                        "test_f1_score": test_metrics["F1-score"],
                    }
                    
                    # Add evaluation metrics to wandb logging if available
...
def main():
    ...
    if config["wandb_opt"]:
        wandb.init(project="craft-train",
                    entity="magisterka_kuchta_geisler",
                    name="craft_training_run",
                    config=config)
