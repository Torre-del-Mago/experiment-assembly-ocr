...
def load_test_dataset_iou(test_folder_name, config, test="val"):
...
def process_single_image(args):
    ...
    k, img_path, model, config, total_imgs_bboxes_gt, result_dir, buffer, gpu_idx, slice_idx = args
    ...
    # Distributed evaluation with thread-safe buffer access
    ...
    if config.vis_opt:
        viz_test(
            ...
            score_text,
            pre_box=polys,
            gt_box=total_imgs_bboxes_gt[k] if k < len(total_imgs_bboxes_gt) else None,
            img_name=img_path,
            result_dir=result_dir,
            test_folder_name="custom_data",
        )
    
    return k, single_img_bbox


def main_eval(model_path, backbone, config, evaluator, result_dir, buffer, model, mode, test="val"):

    if not os.path.exists(result_dir):
        os.makedirs(result_dir, exist_ok=True)

    total_imgs_bboxes_gt, total_imgs_path = load_test_dataset_iou("custom_data", config, test=test)
    ...
    total_imgs_bboxes_pre = [None] * len(piece_imgs_path)
    
    # Prepare arguments for parallel processing
    process_args = [
        (k, img_path, model, config, total_imgs_bboxes_gt, result_dir, buffer, gpu_idx, slice_idx if buffer is not None else 0)
        for k, img_path in enumerate(piece_imgs_path)
    ]
    
    # Use ThreadPoolExecutor for parallel processing
    max_workers = min(8, len(piece_imgs_path))  # Adjust number of workers as needed
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in tqdm(concurrent.futures.as_completed(future_to_idx), 
            ...
            try:
                k, single_img_bbox = future.result()
                total_imgs_bboxes_pre[k] = single_img_bbox
    ...
    results = []
    for i, (gt, pred) in enumerate(tqdm(zip(total_imgs_bboxes_gt, total_imgs_bboxes_pre), desc="Evaluating", total=len(total_imgs_bboxes_gt))):
